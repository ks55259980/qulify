...
@ReportServiceAnnotation(
		name="物流库存报表", id="LogisticsReport", category=Category.Logistics)
public class LogisticsReportService implements ReportService<Map<String, String>> {
	...
	private List<String> columnId = Arrays.asList(
    		"type", "time", "number", "code", "name", "area", "barcode", "startTime", "endTime");
    private List<String> columnName = Arrays.asList(
    		"类型", "入库/发货时间", "批次号/交货单号", "物料代码/经销商代码", "物料名称/经销商名称", "库位/发运路线",
	...
	public MapReport getReport(CodeInfo code) {
		...
		try {
			result = barcodeQueryService.queryByTimeInterval(
					WorkshopUtil.getBarcodeWorkshopCode(code.getWorkshop()),
	...
	public MapReport getReport(String barcode) {
		...
		try {
			result = barcodeQueryService.queryByTimeInterval(
					WorkshopUtil.getBarcodeWorkshopCode("22"),
